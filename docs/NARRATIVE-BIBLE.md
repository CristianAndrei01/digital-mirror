...
|-----------|---------------|
| slope > +0.75σ | **Up** |
| slope between -0.75σ and +0.75σ | **Stable** |
| slope < -0.75σ | **Down** |

σ multiplier (0.75) is configurable per deployment. This prevents false direction signals from low-variance dimensions and catches real shifts in high-variance dimensions.
...
### v2 — Direction Engine
- Expanded numeric view (slopes, sigma, volatility ratios)
