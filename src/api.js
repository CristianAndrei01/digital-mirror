...
module.exports = function createApi(db) {
  ...
  router.get('/dimension/:dim', (req, res) => {
    try {
      ...
      const valid = ['finance', 'health', 'career', 'social', 'family'];
  ...
  router.post('/context-mode', (req, res) => {
    try {
      ...
      res.json({ activated: true, reason, duration, message: `Context mode active for ${duration} days.` });
    } catch (err) {
  ...
  router.get('/status', (req, res) => {
    try {
      const dimensions = getActiveDimensions(db);
      const calibration = {};
      for (const dim of ['finance', 'health', 'career', 'social', 'family']) {
